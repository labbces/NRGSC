...
dea_by_group <- function(metadata, group1, group2, txgen2) {
	...
	res = res[complete.cases(res),]
        ...
        write.table(res, file= paste0("./../results/trans/", group1, "_vs_", group2,"/Trinity.isoform.counts.matrix.", group1, "_vs_", group2, ".DESeq.DE_results"), sep = "\t", quote = F )
	write.table(res, file= paste0("./../results/trans/Trinity.isoform.counts.matrix.", group1, "_vs_", group2, ".DESeq.DE_results"), sep = "\t", quote = F )

        write.table(cbind(as.character(filter$Group), filter$Sample_file), file= paste0("./../results/trans/", group1, "_vs_", group2,"/Trinity.isoform.counts.matrix.", group1, "_vs_", group2, ".DESeq.samples"), col.names = F, row.names = F, sep = "\t", quote = F)
        ...
        ## PlotMA  
        ylim <- c(-10, 10)
        png(paste0("./../results/trans/plotMA-", group1, "-", group2, ".png", sep=""),  width = 15*1.3, height = 15, res = 320, units = "cm", pointsize = 12, bg = "white")
        ...
        ggsave( g, filename = paste0("./../results/trans/volcanoplot-",  group1, "-", group2, ".png"), units = "cm", width = 15*1.3, height = 15, dpi = 320)
        
	filter_3$names <- row.names(filter_3)
	write.table(filter_3, file = paste0("./../results/trans/", group1, "_vs_", group2, "/", group1, "-", group2, "-", "DEA.tsv", sep = ""), sep = "\t", quote = F, col.names = T)
	write.table(filter_3, file = paste0("./../results/trans/",  group1, "-", group2, "-", "DEA.tsv", sep = ""), sep = "\t", quote = F, col.names = T)

        df_fi <- assay(fi)
        df_vst <- assay(vst)	

	write.table(df_fi, file = paste0("./../results/trans/", group1, "_vs_", group2, "/", group1, "-", group2, "-", "filtered_raw_counts.tsv", sep = ""), sep = "\t" ,quote = F, col.names = T)
	write.table(df_fi, file = paste0("./../results/trans/", group1, "-", group2, "-", "filtered_raw_counts.tsv", sep = ""), sep = "\t" ,quote = F, col.names = T)

        write.table(df_vst, file = paste0("./../results/trans/", group1, "_vs_", group2, "/", group1, "-", group2, "-", "vst_counts.tsv", sep = ""), sep = "\t" ,quote = F, col.names = T)
	write.table(df_vst, file = paste0("./../results/trans/", group1, "-", group2, "-", "vst_counts.tsv", sep = ""), sep = "\t" ,quote = F, col.names = T)

	write.table(filter, file = paste0("./../results/trans/",  group1, "_vs_", group2, "/", group1, "-", group2, "-", "metadata.tsv", sep = ""), sep = "\t", quote = F, col.names = T)
	write.table(filter, file = paste0("./../results/trans/",  group1, "-", group2, "-", "metadata.tsv", sep = ""), sep = "\t", quote = F, col.names = T)
}
