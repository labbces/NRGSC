...
results_path_cluster <-"/Storage/data1/jorge.munoz/NRGSC.new/networks/results/perNCONDITION/pearson_GO_1.8/all_GO_modules"
dir.create(results_path_cluster)
# Only to run in local version 
# setwd("/home/jmmunozp/things/GO")
# dir.create(results_path_local)

load_cluster_files <- function(){
## read clusters size
number.clusters <<- read.table("/Storage/data1/jorge.munoz/NRGSC.new/networks/results/perNCONDITION/pearson_mcl/out.1.8.number", header = F)
## read list of genes for all the network 
geneID2GO <<- readMappings(file = "/Storage/data1/jorge.munoz/NRGSC.new/networks/data/go_annotation_gene_ok.txt")
# read genes in graph 
genes_in_graph <<- as.data.frame(rownames(read.table("/Storage/data1/jorge.munoz/NRGSC.new/networks/results/perNCONDITION/pearson_mcl/out.1.8.formated.csv", header =F, row.names=1)))
...
dynamicMods <<- read.table(file = "/Storage/data1/jorge.munoz/NRGSC.new/networks/results/perNCONDITION/pearson_mcl/out.1.8.formated.csv", header = F, row.names=1)
